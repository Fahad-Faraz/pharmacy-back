...
export const convertToBaseUnit = (product, quantity, unitType) => {
  if (product.product_type === "general" || product.product_type === "local" || product.product_type === "franchise") {
    return {
  ...
  if (unitType === "box") {
    baseQty = quantity * product.unit_structure.box.strips * product.unit_structure.strip.tablets;
